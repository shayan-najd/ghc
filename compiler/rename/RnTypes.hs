...
rnWcBody ctxt nwc_rdrs hs_ty
  ...
  where
    ...
    rn_ty env (HsQualTy { hst_ctxt = (dL->(cx , hs_ctxt)), hst_body = hs_ty })
      | Just (hs_ctxt1, hs_ctxt_last) <- snocView hs_ctxt
      , (dL->(lx , HsWildCardTy _))  <- ignoreParens hs_ctxt_last
...
rnHsTyKi env (HsTyVar _ ip (dL->(loc , rdr_name)))
...
rnHsTyOp env overall_ty (dL->(loc , op))
...
bindLHsTyVarBndr :: HsDocContext
                 ...
                 -> (LHsTyVarBndr GhcRn -> RnM (b, FreeVars))
                 -> RnM (b, FreeVars)
bindLHsTyVarBndr _doc mb_assoc (dL->(loc , UserTyVar x lrdr@(dL->(lv , _))))
                 thing_inside
  = do { nm <- newTyVarNameRn mb_assoc lrdr
       ; bindLocalNamesFV [nm] $
         thing_inside (cL loc (UserTyVar x (cL lv nm))) }

bindLHsTyVarBndr doc mb_assoc (dL->(loc , KindedTyVar x lrdr@(dL->(lv, _))kind))
                 thing_inside
...
bindLHsTyVarBndr _ _ _ _ = panic "bindLHsTyVarBndr"

newTyVarNameRn :: Maybe a -> Located RdrName -> RnM Name
newTyVarNameRn mb_assoc (dL->(loc , rdr))
  = do { rdr_env <- getLocalRdrEnv
...
collectAnonWildCards lty = go lty
  where
    go (dL->(_ ,ty)) = case ty of
      HsWildCardTy (AnonWildCard (dL->(_ , wc))) -> [wc]
...
rnField :: FastStringEnv FieldLabel -> RnTyKiEnv -> LConDeclField GhcPs
        -> RnM (LConDeclField GhcRn, FreeVars)
rnField fl_env env (dL->(l , ConDeclField _ names ty haddock_doc))
...
mkHsOpTyRn mk1 pp_op1 fix1 ty1 (dL->(loc2 , HsOpTy noExt ty21 op2 ty22))
  = do  { fix2 <- lookupTyFixityRn op2
        ; mk_hs_op_ty mk1 pp_op1 fix1 ty1
                      (\t1 t2 -> HsOpTy noExt t1 op2 t2)
                      (unLoc op2) fix2 ty21 ty22 loc2 }

mkHsOpTyRn mk1 pp_op1 fix1 ty1 (dL->(loc2 , HsFunTy _ ty21 ty22))
  = mk_hs_op_ty mk1 pp_op1 fix1 ty1
...
-- (e11 `op1` e12) `op2` e2
mkOpAppRn e1@(dL->(_ , OpApp fix1 e11 op1 e12)) op2 fix2 e2
  | nofix_error
...
---------------------------
--      (- neg_arg) `op` e2
mkOpAppRn e1@(dL->(_ , NegApp _ neg_arg neg_name)) op2 fix2 e2
  | nofix_error
...
get_op (dL->(_ , HsVar _ (dL->(_ , n)))) = NormalOp n
get_op (dL->(_ , HsUnboundVar _ uv)) = UnboundOp uv
get_op (dL->(_ , HsRecFld _ fld))    = RecFldOp fld
...
-- (e11 `op1` e12) `op2` e2
mkOpFormRn a1@(dL->(loc , HsCmdTop _ (dL->(_ , HsCmdArrForm x op1 f (Just fix1)
                                     [a11,a12]))))
        op2 fix2 a2
  | nofix_error
  = do precParseErr (get_op op1,fix1) (get_op op2,fix2)
...
checkPrecMatch op (MG { mg_alts = (dL->(_ , ms)) })
  = mapM_ check ms
  where
    check :: Located (Match GhcRn body) -> TcRn ()
    check (dL->(_ , Match { m_pats = (dL->(l1 , p1)) : (dL->(l2 , p2)) :_ }))
...
warnUnusedForAll in_doc (dL->(loc , tv)) used_names
...
extract_lty :: TypeOrKind -> LHsType GhcPs
            -> FreeKiTyVarsWithDups -> FreeKiTyVarsWithDups
extract_lty t_or_k (dL->(_ , ty)) acc
  = case ty of
...
extract_hs_tv_bndrs_kvs tv_bndrs
  = freeKiTyVarsKindVars $        -- There will /be/ no free tyvars!
    foldr extract_lkind emptyFKTV
          [k | (dL->(_ ,KindedTyVar _ _ k)) <- tv_bndrs]

extract_tv :: TypeOrKind -> Located RdrName
           -> FreeKiTyVarsWithDups -> FreeKiTyVarsWithDups
extract_tv t_or_k ltv@(dL->(_ , tv)) acc@(FKTV kvs tvs)
